    ...
    (let loop ((from from)
      ...
      (if (eq? (csc-buffer from) to-buffer)
          ...
          (loop (char-seq-cursor-next from)
                (append! prefix (string->list (csc-buffer from))))))))
...
(define (->raw-input location x chunk-size)
  (cond ((lazy-seq? x) x)
        ((parser-input? x) (parser-input-raw x))
        ((string? x) (char-seq-cursor x #f))
        ((list? x) (list->lazy-seq x))
        ((input-port? x) (char-seq-cursor x chunk-size))
        (else (error location "Unable to convert object to parser input" x))))

(define default-parser-input-chunk-size
  (make-parameter 1024))

(define (->parser-input x #!key (chunk-size (default-parser-input-chunk-size)))
  (make-parser-input (->raw-input '->parser-input x chunk-size)))

(define (parse parser input #!key memoize (chunk-size (default-parser-input-chunk-size)))
  (parameterize ((memo-table (if memoize (make-hash-table) (memo-table))))
    (let* ((raw-input (->raw-input 'parse input chunk-size))
